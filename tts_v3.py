...
class TTS:
    ...
    def start(self, language: str = DEFAULT_LANGUAGE) -> TTS:
        ...
        while time.time() < deadline:
            ...
            with open(self._log_path, "r", encoding="utf-8", errors="replace") as lf:
                lf.seek(pre_size)
                for line in lf:
                    if " server.ready" in line and "| v3" in line:
    ...
    def synthesize(self, text: str, language: str = DEFAULT_LANGUAGE) -> Path:
        self.start(language)
        pieces = self._chunks(text)
        output = ROOT / f"out_{time.strftime(TIMESTAMP_FORMAT)}_v3.wav"
...
if __name__ == "__main__":
    ...
    p.add_argument("--language", default=DEFAULT_LANGUAGE, help="ISO 639-1 language code (e.g. en, fr, zh)")
    ...
    wav_info.close()
    print(f"[rtf] v3_start={t_start:.3f}s", file=sys.stderr)
    print(f"[rtf] v3_synth={t_done-t_start:.3f}s", file=sys.stderr)
    print(f"[rtf] v3_total={t_done-t_start:.3f}s", file=sys.stderr)
