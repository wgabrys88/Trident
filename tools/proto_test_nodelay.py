"""Test with TCP_NODELAY and proper flushing."""
import socket
import struct
import subprocess
import sys
...
cmd = [
    str(TTS_EXE), "--run-id", "proto-test2", "--family", "v3",
...
print(f"connected, TCP_NODELAY=1")
...
print("sent, sleeping 2s for chatterbox to process...")
...
try:
    ...
    if len(data) == 32:
        ...
        print(f"got response: magic={magic:08x} ver={ver} kind={kind} chunk={c} length={length}")
