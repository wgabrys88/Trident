...
ggml_tensor* S3::transformer(ggml_context* ctx, ggml_tensor* x, const std::string& name, int frames) const {
    ...
    auto projection = [&](const char* suffix) {
        ...
        return ggml_view_4d(ctx, value, head, frames, heads, x->ne[2], width * sizeof(float), head * sizeof(float), size_t(width) * frames * sizeof(float), 0);
    };
    ...
    auto* flat = ggml_reshape_3d(ctx, attention, width, frames, x->ne[2]);
