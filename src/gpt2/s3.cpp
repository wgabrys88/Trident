...
ggml_tensor* MeanflowS3::transformer(ggml_context* ctx, ggml_tensor* x, const std::string& name, int frames) const {
    ...
    auto projection = [&](const char* suffix) {
        ...
        return ggml_view_3d(ctx, value, 64, frames, 8, 512 * sizeof(float), 64 * sizeof(float), 0);
    };
    ...
    auto* flat = ggml_reshape_2d(ctx, attention, 512, frames);
...
std::vector<float> MeanflowS3::estimate(const std::vector<float>& state, const std::vector<float>& mu,
    ...
    if (estimator_frames_ != frames) {
        ...
        auto* x = input("x", frames, 80); auto* mean = input("mu", frames, 80);
        auto* spk = input("speaker", 80, 1); auto* cond = input("condition", frames, 80); auto* t = input("time", 1024, 1);
        auto* repeated = ggml_repeat(ctx, ggml_reshape_2d(ctx, spk, 1, 80), x);
