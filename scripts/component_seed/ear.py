...
from components.runtime import MODELS, ROOT, WORK, Http
...
class Ear:
    def __init__(self, server: str, model: bool = True):
    ...
    def send(self, text: str, language: str = "", timestamps: str = "", last=None):
        ...
        if not text:
            return
        row = self.http.call("POST", "/heard", {"text": text, "language": language, "timestamps": timestamps})
    ...
    def drain_inbox(self):
        ...
        for path in sorted(inbox.glob("*.txt")):
            ...
            if not body:
                self.http.call("POST", "/archive", {"path": str(path.relative_to(ROOT)), "kind": "inbox"})
                continue
    ...
    def hear_file(self, path: str):
        ...
        language, text, times, _ = self.transcribe_utterance([audio], pull)
        if not text:
            raise RuntimeError("ear heard nothing")
        self.send(text, language, times)

    def run(self, mic: bool = True):
        self.http.call("POST", "/ready", {"name": "ear"})
        if not mic:
            while True:
                self.drain_inbox()
                time.sleep(0.05)
            ...
            while True:
                ...
                def pull():
                    ...
                    if float(np.abs(chunk).mean()) >= level:
                        ...
                        return chunk
                    quiet["n"] += 1
                    return None if quiet["n"] >= pause else chunk
