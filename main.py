...
from config import (
    ...
    file_sha256, find_exe, git_identity, load_settings, wasapi_device,
)
import config
...
def _need(parser, bad: bool, msg: str) -> None:
    if bad: parser.error(msg)
...
def _manifest(paths: Paths) -> dict:
    ...
    if paths.command != "install":
        products = product_stamps(paths.models_dir)
        manifest["products"] = products
        if paths.command in ("talk", "tts"):
            exe = find_exe(paths.models_dir / "tts", "chatterbox-server.exe")
            if exe is None: raise RuntimeError("chatterbox-server.exe missing; run python main.py install")
            manifest["tts_binary"] = {"path": str(exe), "sha256": file_sha256(exe), "stamp": products.get("tts", "")}
    return manifest
...
def main(command: str | None = None) -> int:
    ...
    if cmd in ("tts", "generation"):
        _need(parser, cmd == "generation" and any(v is not None for v in content_flags[2:]), "generation does not accept interrupt flags")
        _need(parser, (args.text is None) == (args.text_file is None), "exactly one of --text and --text-file is required")
        if cmd == "tts":
            _need(parser, args.interrupt_text is not None and args.interrupt_file is not None, "--interrupt-text and --interrupt-file are mutually exclusive")
            _need(parser, (args.interrupt_text is not None or args.interrupt_file is not None) != (args.interrupt_after is not None), "interrupt content and --interrupt-after are required together")
            _need(parser, args.interrupt_after is not None and (not math.isfinite(args.interrupt_after) or args.interrupt_after < 0), "--interrupt-after must be finite and non-negative")
        ...
        _need(parser, not primary, "TTS input is empty" if cmd == "tts" else "generation input is empty")
        _need(parser, replacement is not None and not replacement, "TTS replacement is empty")
    else:
        _need(parser, any(v is not None for v in content_flags),
              "TTS text and replacement flags require command tts" if cmd == "talk" else f"{cmd} does not accept streaming or TTS content flags")
    ...
    _need(parser, HARDWARE == "irisxe" and cmd in ("talk", "tts") and family != "nano", "Iris Xe supports Nano English only")
    _need(parser, cmd not in ("talk", "tts") and any(v is not None for v in tts_options), "TTS performance overrides require command talk or tts")
    _need(parser, cmd not in ("talk", "generation") and any(v is not None for v in gemma_options), "Gemma options require command talk or generation")
    _need(parser, args.system_prompt is not None and args.system_prompt_file is not None, "--system-prompt and --system-prompt-file are mutually exclusive")
    _need(parser, cmd not in ("talk", "asr") and args.asr_device is not None, "--asr-device requires command talk or asr")
    _need(parser, cmd != "talk" and args.prefill_min_words is not None, "--prefill-min-words requires command talk")
    _need(parser, args.voice is not None and not args.voice.strip(), "--voice must not be empty")
    _need(parser, args.gemma_device is not None and not args.gemma_device.strip(), "--gemma-device must not be empty")
    _need(parser, args.asr_device is not None and not args.asr_device.strip(), "--asr-device must not be empty")
    _need(parser, cmd in ("talk", "asr") and language not in ASR_LOCALES,
          f"streaming Parakeet has no validated locale for {language!r}; use a supported ASR language")
    for value, lo, hi, msg in (
        (args.tts_threads, 1, None, "--tts-threads must be positive"),
        (args.cfm_steps, 5 if family == "v3" else 1, None, "--cfm-steps must be at least 5 for V3 and at least 1 otherwise"),
        (args.cfg_weight, 0, None, "--cfg-weight must be non-negative"),
        (args.exaggeration, 0, None, "--exaggeration must be non-negative"),
        (args.gemma_context, 2048, 131072, "--gemma-context must be between 2048 and 131072"),
        (args.gemma_max_tokens, 1, None, "--gemma-max-tokens must be positive"),
        (args.gemma_top_p, 0, 1, "--gemma-top-p must be between 0 and 1"),
        (args.gemma_min_p, 0, 1, "--gemma-min-p must be between 0 and 1"),
        (args.gemma_temperature, 0, None, "--gemma-temperature must be non-negative"),
        (args.gemma_top_k, 0, None, "--gemma-top-k must be non-negative"),
        (args.gemma_cache_ram, 0, None, "--gemma-cache-ram must be non-negative"),
        (args.gemma_ctx_checkpoints, 0, None, "--gemma-ctx-checkpoints must be non-negative"),
        (args.gemma_checkpoint_min_step, 0, None, "--gemma-checkpoint-min-step must be non-negative"),
        (args.gemma_parallel, 1, None, "--gemma-parallel must be positive"),
        (args.gemma_threads, 1, None, "--gemma-threads must be positive"),
        (args.gemma_threads_batch, 1, None, "--gemma-threads-batch must be positive"),
        (args.gemma_batch_size, 1, None, "--gemma-batch-size must be positive"),
        (args.gemma_ubatch_size, 1, None, "--gemma-ubatch-size must be positive"),
        (args.thinking_budget, -1, None, "--thinking-budget must be -1 or non-negative"),
        (args.history_turns, 1, None, "--history-turns must be positive"),
        (args.prefill_min_words, 1, None, "--prefill-min-words must be positive"),
    ):
        if value is not None and (value < lo or (hi is not None and value > hi)): parser.error(msg)
    _need(parser, args.gemma_repeat_penalty is not None and args.gemma_repeat_penalty <= 0, "--gemma-repeat-penalty must be positive")
