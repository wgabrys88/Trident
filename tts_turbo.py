...
CHECKPOINT_FILES = ("t3_turbo_v1.safetensors", "s3gen_meanflow.safetensors", "conds.pt",
...
def _convert(work: Path, source: Path) -> None:
    ...
    for name in CHECKPOINT_FILES:
        _download(f"{TURBO_URL}/{name}", checkpoint / name)
    for script, model_args, output in (
        (CONVERTERS[0], ("--model", "turbo"), T3),
...
def _from_hf() -> None:
    ...
    pull("turbo", T3.name, T3)
    pull("turbo", S3.name, S3)
    if not (MODELS / "turbo-model-card.md").is_file():
        pull("turbo", "README.md", MODELS / "turbo-model-card.md")
...
def _install() -> None:
    ...
    models = [T3, S3, MODELS / "turbo-model-card.md"]
    ...
    if runtime_ok and models_ok and voice_ok:
        ...
        with tempfile.TemporaryDirectory(prefix=".turbo-install-", dir=ROOT) as tmp:
            work = Path(tmp)
            ...
            if not models_ok:
                ...
                _download(f"{TURBO_URL}/README.md", MODELS / "turbo-model-card.md")
            if not voice_ok:
